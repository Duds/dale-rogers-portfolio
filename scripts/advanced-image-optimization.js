...
async function optimizeImageAdvanced(imagePath) {
  ...
  const optimizedPath = join(
    outputDir,
    `${fileName}-optimized.${format === "jpeg" ? "jpg" : format}`,
  );
...
async function generateImageManifest(optimizedImages) {
  ...
  manifest.savingsPercent = (
    (manifest.savings / manifest.totalOriginalSize) *
    100
  ).toFixed(1);
...
async function main() {
  ...
  for (const imagePath of imageFiles) {
    try {
      ...
      logSuccess(
        `Processed ${processedCount}/${imageFiles.length}: ${basename(imagePath)}`,
      );

      // Show optimization results
      const originalSize = result.original.size;
      const bestOptimized = result.optimized.find(
        (v) => v.format === "webp" && v.size === "fallback",
      );
      ...
      if (bestOptimized) {
        ...
        logInfo(
          `  Original: ${formatBytes(originalSize)} → Optimized: ${formatBytes(bestOptimized.fileSize)} (${savedPercent}% saved)`,
        );
      }
    } catch (error) {
  ...
  logSuccess(
    `Total space saved: ${formatBytes(manifest.savings)} (${manifest.savingsPercent}%)`,
  );
