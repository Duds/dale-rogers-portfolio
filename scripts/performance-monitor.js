...
function analyzeBundlePerformance() {
  ...
  if (existsSync(clientPath)) {
    ...
    const clientAssessment = assessPerformance('clientBundle', clientSize, PERFORMANCE_BUDGETS.clientBundle);
    ...
    logMetric('  Size', results.client.sizeFormatted, '', clientAssessment.status);
    logInfo(`  Status: ${clientAssessment.message}`);
    
    // Analyze individual files
    try {
      const clientFiles = execSync(`find "${clientPath}" -type f -name "*.js"`, { encoding: 'utf8' }).split('\n').filter(Boolean);
      
      for (const file of clientFiles) {
        const fileSize = statSync(file).size;
        const fileName = file.replace(clientPath + '/', '');
        const fileAssessment = assessPerformance('individualFile', fileSize, PERFORMANCE_BUDGETS.individualFile);
        ...
        if (fileAssessment.status !== 'good') {
          logWarning(`  ${fileName}: ${formatBytes(fileSize)} - ${fileAssessment.message}`);
        }
      }
    } catch (error) {
  ...
  if (existsSync(serverPath)) {
    ...
    const serverAssessment = assessPerformance('serverBundle', serverSize, PERFORMANCE_BUDGETS.serverBundle);
    ...
    logMetric('  Size', results.server.sizeFormatted, '', serverAssessment.status);
...
function generatePerformanceReport(bundleResults, buildTime) {
  ...
  const statuses = [
    ...
    buildTime ? assessPerformance('buildTime', buildTime, PERFORMANCE_BUDGETS.buildTime).status : 'good'
  ].filter(Boolean);
  ...
  const largeFiles = bundleResults?.files?.filter(f => f.status !== 'good') || [];
  if (largeFiles.length > 0) {
    report.summary.recommendations.push(`Optimize ${largeFiles.length} large files`);
  }
  
  // Display summary
  logInfo('Overall Status:');
  const statusColor = report.summary.overallStatus === 'good' ? 'green' : 
                     report.summary.overallStatus === 'warning' ? 'yellow' : 'red';
  ...
  try {
    ...
    if (latest.bundleAnalysis?.client && previous.bundleAnalysis?.client) {
      ...
      if (change > 0) {
        ...
        logSuccess(`  Decreased by ${formatBytes(Math.abs(change))} (${Math.abs(changePercent)}%)`);
      } else {
        logInfo('  No change');
      }
    }
    ...
    if (latest.buildTime && previous.buildTime) {
      ...
      if (change > 0) {
        ...
        logSuccess(`  Decreased by ${Math.abs(change).toFixed(1)}s (${Math.abs(changePercent)}%)`);
      } else {
        logInfo('  No change');
      }
    }
...
async function main() {
  ...
  if (buildTime !== null) {
    const buildAssessment = assessPerformance('buildTime', buildTime, PERFORMANCE_BUDGETS.buildTime);
