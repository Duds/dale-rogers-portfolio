...
function logMenu() {
  ...
  console.log(
    "12. 🚀 Advanced Image Optimization - Generate responsive variants",
  );
...
function runCommand(command, description) {
  ...
  try {
    ...
    log(
      `${colors.red}❌ ${description} failed: ${error.message}${colors.reset}`,
    );
  }
}
...
function storybook() {
  const child = runInteractiveCommand(
    "pnpm run storybook",
...
function advancedImageOptimization() {
  runCommand(
    "node scripts/advanced-image-optimization.js",
...
function quickStart() {
  ...
  if (!existsSync(join(projectRoot, "node_modules"))) {
    log(
      `${colors.cyan}No node_modules found, running clean install...${colors.reset}`,
    );
    cleanInstall();
  } else {
...
function showProjectInfo() {
  try {
    ...
    log(
      `${colors.cyan}Node.js: ${packageJson.engines?.node || "Not specified"}${colors.reset}`,
    );
    ...
    log(
      `${colors.cyan}Tailwind CSS: ${hasTailwindV4 ? "v4 ✅" : "v3 or earlier"}${colors.reset}`,
    );
    log(
      `${colors.cyan}Vite Plugin: ${hasVitePlugin ? "Present ✅" : "Missing ❌"}${colors.reset}`,
    );
  } catch (error) {
    log(
      `${colors.red}Failed to read project info: ${error.message}${colors.reset}`,
    );
  }
}
  ...
  while (running) {
    ...
    try {
      const answer = await askQuestion(
        `${colors.cyan}Select an option (0-9): ${colors.reset}`,
      );
