...
- Name: ThemeToggle component renders and can be toggled
- Location: /home/dale-rogers/dev/personal/dale-rogers-portfolio/tests/ui/themetoggle.spec.ts:3:1
...
Locator: getByRole('button', { name: /theme|toggle/i })
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByRole('button', { name: /theme|toggle/i })

    at /home/dale-rogers/dev/personal/dale-rogers-portfolio/tests/ui/themetoggle.spec.ts:6:24
```
...
```ts
   ...
   2 |
   3 | test("ThemeToggle component renders and can be toggled", async ({ page }) => {
   4 |   await page.goto("/");
   5 |   const toggle = await page.getByRole("button", { name: /theme|toggle/i });
>  6 |   await expect(toggle).toBeVisible();
     |                        ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
   7 |   await toggle.click();
   8 |   // Optionally, check for theme change
   9 | });
  10 |
```