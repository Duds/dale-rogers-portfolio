...
- Name: TagList component renders tags as links
- Location: /home/dale-rogers/dev/personal/dale-rogers-portfolio/tests/ui/taglist.spec.ts:3:1
...
Locator: getByRole('link', { name: /design|accessibility|ux/i })
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByRole('link', { name: /design|accessibility|ux/i })

    at /home/dale-rogers/dev/personal/dale-rogers-portfolio/tests/ui/taglist.spec.ts:9:21
```
...
```ts
   ...
   2 |
   3 | test("TagList component renders tags as links", async ({ page }) => {
   4 |   await page.goto("/");
   5 |   // Adjust tag names as needed
   6 |   const tag = await page.getByRole("link", {
   7 |     name: /design|accessibility|ux/i,
   8 |   });
>  9 |   await expect(tag).toBeVisible();
     |                     ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
  10 | });
  11 |
```